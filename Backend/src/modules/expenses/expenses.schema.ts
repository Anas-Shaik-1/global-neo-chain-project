...
// User-submittable categories — SALARY is system-generated by payroll and
// must not be selectable when creating an expense via the API.
const USER_EXPENSE_CATEGORIES = EXPENSE_CATEGORIES.filter(
  (c) => c !== "SALARY",
) as Exclude<(typeof EXPENSE_CATEGORIES)[number], "SALARY">[];
...
export const CreateExpenseBody = z
  .object({
    ...
    category: z.enum(
      USER_EXPENSE_CATEGORIES as unknown as [string, ...string[]],
    ),
